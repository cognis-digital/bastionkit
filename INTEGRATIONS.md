...
**bastionkit** plugs into your stack through [`cognis-connect`](https://github.com/cognis-digital/cognis-connect),
...
Once `bastionkit` emits JSON findings, pipe them straight to a destination — `--dry-run`
previews the exact request without sending:

```bash
bastionkit ... --format json | cognis-connect emit --to sigma   # Sigma rules
bastionkit ... --format json | cognis-connect emit --to splunk --url $URL --token $TOK   # Splunk HEC
bastionkit ... --format json | cognis-connect emit --to elastic --url $URL --token $TOK   # Elastic _bulk
bastionkit ... --format json | cognis-connect emit --to webhook --url $URL --token $TOK   # generic webhook
```
...
works whatever `bastionkit` outputs:

```python
from cognis_connect import normalize, sigma
findings = [normalize(rec, source="bastionkit") for rec in records]   # records = your JSON output
